...
std::vector<std::string> parse(const std::string& line, std::regex reg) {
  return std::vector<std::string>(std::sregex_token_iterator(line.begin(), line.end(), reg),
...
bool operational(char s) {
  return s == '.' || s == '?';
}

size_t arrangements(const record_t& record,
                    size_t spring_n = 0,
                    size_t run_n    = 0,
                    size_t group_n  = 0,
  ...
  if (spring_n == record.springs.size()) {  //end of record
    bool valid_arrangement =
      (run_n == 0 && group_n == record.groups.size()) ||
      (group_n == record.groups.size() - 1 && record.groups[group_n] == run_n);
  ...
  if (operational(record.springs[spring_n])) {
    ...
    } else if (group_n < record.groups.size() &&
               record.groups[group_n] == run_n) {  //valid new group
...
size_t part2(const records_t& records) {
  ...
  return std::accumulate(
    unfolded_records.begin(), unfolded_records.end(), 0ull, [](auto& sum, auto& record) {
      return sum + arrangements(record);
    });
}

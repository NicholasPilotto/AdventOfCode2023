...
void solve() {
  ...
  for (unsigned i = 0; i < galaxies.size(); i++) {
    for (unsigned j = i + 1; j < galaxies.size(); j++) {
      ...
      expansion += std::count_if(empty_rows.begin(), empty_rows.end(), [r_lo, r_hi](unsigned r) {
        return r > r_lo && r < r_hi;
      });
      //Count the number of empty cols in between the galaxies.
      expansion += std::count_if(empty_cols.begin(), empty_cols.end(), [c_lo, c_hi](unsigned c) {
      ...
      p2 += r_hi - r_lo + c_hi - c_lo + expansion * 999'999;
    }
  }

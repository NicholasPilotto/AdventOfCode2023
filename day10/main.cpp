...
unsigned explore(Pos cur, Dir from_dir) {
  try {
    ...
    switch (pipes.at(cur) & (~from_dir)) {
      case E:
        return 1 + explore({cur.first, cur.second + 1}, W);
      case N:
        return 1 + explore({cur.first - 1, cur.second}, S);
      case S:
        return 1 + explore({cur.first + 1, cur.second}, N);
      case W:
        return 1 + explore({cur.first, cur.second - 1}, E);
    }
  } catch (const std::out_of_range& oor) {}
...
void solve() {
  ...
  std::set<std::pair<Pos, Dir>> neighbors{
    {{start.first.first, start.first.second + 1}, W},
